    ...
    <div className="min-h-screen bg-brand-bg text-brand-text pb-28 md:pb-10">
      <div className="w-full px-4 md:px-6 pt-6 md:pt-8 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div>
            <h2 className="text-2xl font-black text-brand-text tracking-tight">Bookings</h2>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {/* View toggle */}
            <div className="inline-flex bg-brand-bg border border-brand-border/40 rounded-xl p-1">
              <button
                onClick={() => setViewMode('list')}
                className={`h-7 px-3 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${
                  viewMode === 'list' ? 'bg-brand-primary text-white' : 'text-brand-muted hover:text-brand-primary'
                }`}
              >
                List
              </button>
              <button
                ...
                className={`h-7 px-3 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${
                  viewMode === 'calendar' ? 'bg-brand-primary text-white' : 'text-brand-muted hover:text-brand-primary'
                }`}
              >
                Calendar
              </button>
            ...
            <span className="px-3 py-1.5 bg-brand-primary/10 border border-brand-primary/20 rounded-xl text-[11px] font-bold text-brand-primary">
            ...
            <button
              ...
              className="h-9 px-3 rounded-xl bg-brand-primary hover:bg-brand-primary/90 text-[11px] font-black text-white inline-flex items-center transition-all"
            >
              New booking
            </button>
          </div>
        ...
        {/* Filter card */}
        <div className="bg-brand-surface rounded-xl p-4 md:p-5 border border-brand-border/20">

          <div className="flex flex-col md:flex-row gap-3">
            {/* Search */}
            <div className="relative flex-1">
              <span className="material-symbols-outlined absolute left-3.5 top-1/2 -translate-y-1/2 text-brand-muted text-[20px]">search</span>
              <input
                type="text"
                placeholder="Search by booking ID or customer name..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="w-full h-11 bg-brand-bg border border-brand-border/40 rounded-xl pl-11 pr-4 text-sm font-semibold text-brand-text placeholder:text-brand-muted focus:ring-2 focus:ring-brand-primary/20 focus:border-brand-primary/40 outline-none transition-all"
              />
            </div>

            {/* Status dropdown */}
            <div className="relative">
              <span className="material-symbols-outlined absolute left-3.5 top-1/2 -translate-y-1/2 text-brand-muted text-[18px] pointer-events-none">filter_list</span>
              <select
                value={selectedStatus ?? ''}
                onChange={(e) => setSelectedStatus(e.target.value === '' ? null : e.target.value)}
                className="h-11 w-full md:w-52 bg-brand-bg border border-brand-border/40 rounded-xl pl-11 pr-10 text-sm font-semibold text-brand-text focus:ring-2 focus:ring-brand-primary/20 focus:border-brand-primary/40 outline-none transition-all appearance-none cursor-pointer [color-scheme:dark]"
              >
            ...
            {/* Staff dropdown */}
            <div className="relative">
              <span className="material-symbols-outlined absolute left-3.5 top-1/2 -translate-y-1/2 text-brand-muted text-[18px] pointer-events-none">person</span>
              <select
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-28 gap-4">
            ...
            <p className="text-brand-muted text-sm font-semibold">Loading bookings...</p>
          </div>
        )}
        ...
        {!loading && viewMode === 'list' && filteredBookings.length === 0 && (
          <div className="flex flex-col items-center justify-center py-28 gap-4 text-center">
            <div className="w-16 h-16 rounded-2xl bg-brand-surface border border-brand-border/30 flex items-center justify-center">
              <span className="material-symbols-outlined text-3xl text-brand-muted">event_busy</span>
            </div>
            <div>
              <p className="text-brand-primary font-black">No bookings found</p>
